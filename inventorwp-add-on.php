		        ...
		        array(
		                ...
		                'banner_video' => array(
		                        ...
		                        $inventorwp_addon->add_field(
								    ...
								    'If using \'Set with XPath\' the value must be either \'on\' for \'Yes\' or empty for \'No\'.'
								)
		                ),
		        )
		);
	...
	case 'car':
		...
		**/
		...
		$inventorwp_addon->add_field(
		    ...
		    'If using \'Set with XPath\' the value must be either \'NEW\' or \'USED\'. Case sensitive.'
		);
		$inventorwp_addon->add_field(
		    'listing_car_leasing',
		    'Leasing Available',
	...
	case 'dating':
		...
		**/
		...
		$inventorwp_addon->add_field(
		    ...
		    'If using \'Set with XPath\' the value must be either \'MALE\' or \'FEMALE\'. Case sensitive.'
		);
...
function inventorwp_import($post_id, $data, $import_options, $article, $logger) {
    ...
    if (
	    ...
	    $listing_location_serialized = serialize($listing_location);

    	// Update inside view array
    	update_post_meta( $post_id, $field, $listing_location_serialized );
    ...
    if(
    	...
    	// The sub child field has a value
    	&& isset($data['tmp_locations_sub_child'])
    	&& !empty($data['tmp_locations_sub_child']))
		|| 
		...
		&& !empty($data['tmp_locations_sub_child']))
    		...
    		update_post_meta( $post_id, 'listing_locations', sanitize_title($data['tmp_locations_sub_child']));
		...
		// The sub child field is empty
		&& !isset($data['tmp_locations_sub_child'])
		&& empty($data['tmp_locations_sub_child']) 

		// The child field has a value
		&& isset($data['tmp_locations_child']) 
		&& !empty($data['tmp_locations_child']))
		||
		// Updating the post is allowed
		($inventorwp_addon->can_update_meta('listing_locations', $import_options) 
		// The sub child field is empty
		...
		&& !empty($data['tmp_locations_child']))
			...
			update_post_meta( $post_id, 'listing_locations', sanitize_title($data['tmp_locations_child']));
		...
		&& empty($data['tmp_locations_child']))
   		...
   		update_post_meta( $post_id, 'listing_locations', sanitize_title($data['tmp_locations_parent']));
